...
/*
 * this class is used as handler for internal server to server communication
 * */
...
public class CommHandler extends SimpleChannelInboundHandler<eye.Comm.Request> {
	...
	public CommHandler(PerChannelQueue q) {
		this.sq = q;
	}
	...
	protected void channelRead0(ChannelHandlerContext ctx, eye.Comm.Request msg)
		...
		int node = msg.getHeader().getToNode();

		int requestCount = RequestCountMap.getReqCntMap().get(node);
		System.out.println("Node : " + node + "has " + requestCount
				+ " requests");

		RequestCountMap.getReqCntMap().put(node, requestCount - 1);

		//every response is checked weather it was success or failure.
		//if success it is sent back to client and if failure than forwards 
		//to other cluster ...this is handled in method used below
		sq.forwardToOtherCluster(msg);
