...
public class HeartbeatManager extends Thread {
    ...
    public void setCpuUsage(float cpuUsage) {
            ...
            HeartbeatManager.getInstance().cpuUsage = cpuUsage;
    }
	...
	public void processRequest(Management mgmt) {
		...
		if (hd == null) {
			...
			logger.info("System usage for "+ hd.getNodeId() +" : "+ hd.getHost() +" : " + hd.getSystemUsage()
					+ " curret leader is " + ElectionManager.getInstance().whoIsTheLeader());
			
			//current system usage is extracted from heartbeats 
			hd.setSystemUsage(cpuUsage);
			hd.setFailures(0);
	...
	private Management generateHB() {
		...
		//current system usage is embedded in hearbeats to send to leader node
		h.setUsage(cpuUsage);
		///////////
	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				...
				if(RequestCountMap.getReqCntMap()!= null && RequestCountMap.getReqCntMap().containsKey(heart.getNodeId()))
					RequestCountMap.getReqCntMap().remove(heart.getNodeId());
					
				//only restarts election if leader node is down. 	
