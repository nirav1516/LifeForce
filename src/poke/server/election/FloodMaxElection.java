...
import poke.server.usage.changeIp;
...
public class FloodMaxElection implements Election {
	...
	public Management process(Management mgmt) {
		...
		if (req.getAction().getNumber() == ElectAction.DECLAREELECTION_VALUE) {
			...
			// Changing leaderIP in the heartbeat data of the adjacent nodes.
			/*
			 * if(req.getHops() > 200){
			 * HeartbeatManager.getInstance().incomingHB
			 * .get(req.getCandidateId()).setHost("10.0.0.8"); }
			 */
	...
	private Management castVote(Management mgmt, boolean isNew) {
		...
		if (req.getExpires() <= System.currentTimeMillis()) {
			logger.info("Node " + this.nodeId
					+ " says election expired - not voting");
			return null;
		}
		...
		if (req.getCandidateId() == this.nodeId) {
			...
			elb.setHops(mgmt.getHeader().getPathCount());// CHANGE
			// elb.setHops(1);
			// If the election is over then set the leaders IP to the
			// predetermined leaderIp
			/*
			 * if (elb.getHops() <= 0) {
			 * System.out.println("--------Going to change my ip------------");
			 * changeIp.changeip(); }
			 */
			// logger.info("Node " + this.nodeId +
			...
			if (req.getHops() == -1) {
				//System.out.println("Infinite voting");
				elb.setHops(-1);
			} else{
				elb.setHops(req.getHops() - 1);
				System.out.println("Decrement hops");
			}
			...
			if (elb.getHops() == 0) {
				...
				System.out.println("my hops are zero : header path count :" + mgmt.getHeader().getPathCount());
				elb.setHops(mgmt.getHeader().getPathCount()); // CHANGE
				// elb.setHops(1);
