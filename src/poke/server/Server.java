...
import poke.server.usage.SystemUsage;
...
public class Server {
	...
	protected SystemUsage sysusage;
	...
	private void init(File cfg) {
		...
		try {
			...
			ForwardResource.initialize(conf);
			ResourceFactory.initialize(conf);
			//change - Initialize database connection 
			MongoConnection.initConnection(conf);
	...
	private void startManagers() {
		...
		//we are turning on the thread to constantly monitor the current system usage 
		sysusage = SystemUsage.init();
		sysusage.Initialize();
